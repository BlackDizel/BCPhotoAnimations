...
public class DialogExportMediaCodec implements IDialog, View.OnClickListener {
    private final AlertDialog dialog;

    @Inject
    IPresenterProjectExportMediaCodec presenter;
    private TextView tvFps, tvWidth, tvHeight;

    public DialogExportMediaCodec(Context context) {
        ApplicationStopMotion.getComponent().inject(this);
        dialog = new AlertDialog.Builder(context)
                .create();

        View view = LayoutInflater.from(context).inflate(R.layout.dialog_export_mediacodec, null);
